...
buildah_using_from_scratch() {
    ...
    echo
    read_red_color "$ sudo ls \$scratchmnt"
    sudo ls $scratchmnt
    read -p "Enter to continue"
    ...
    echo
    read_red_color "$ sudo buildah run \$newcontainer -- ls -alF /usr/local/bin"
...
EOF
    ...
    echo
    read_red_color "$ sudo buildah copy \$newcontainer ./runecho.sh /usr/local/bin"
    sudo buildah copy $newcontainer ./runecho.sh /usr/local/bin

    echo
    echo_color "Set the cmd of the container to the script"
    echo
    read_red_color "$ sudo buildah config --cmd /usr/local/bin/runecho.sh \$newcontainer"
    ...
    echo
    read_red_color "$ sudo buildah run $newcontainer /usr/local/bin/runecho.sh"
    ...
    echo
    read_red_color "$ sudo buildah config --created-by \"buildahdemo\"  \$newcontainer"
    ...
    echo
    read_red_color "$ sudo buildah rm \$newcontainer"
...
run_image_in_docker() {
    ...
    echo
    read_red_color "$ sudo buildah push fedora-bashecho docker-daemon:fedora-bashecho:latest"
    sudo buildah push fedora-bashecho docker-daemon:fedora-bashecho:latest

    echo
    echo_color "Show our image under Docker"
    echo
    read_red_color "$ sudo docker images"
    sudo docker images

    echo
    echo_color "Run our image under Docker"
    echo
    read_red_color "$ sudo docker run fedora-bashecho"
...
EOF
    ...
    echo
    read_red_color "$ sudo buildah bud -t hello -f ./Dockerfile.hello ."
    sudo buildah bud -t hello -f ./Dockerfile.hello .

    echo
    echo_color "Create the container from the image"
    echo
    read_red_color "$ sudo buildah from hello"
    sudo buildah from hello

    echo
    echo_color "Run the container"
    echo
    read_red_color "$ sudo buildah run hello-working-container python3 /home/HelloFromContainer.py"
    ...
    echo
    read_red_color "$ sudo podman run hello"
...
clean_images_and_containers() {
    ...
    read_red_color "$ sudo buildah rm -a"
    sudo buildah rm -a
    echo
    read_red_color "$ sudo buildah rmi -a -f"
